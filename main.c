...
int main(int argc, char* argv[])
{
    ...
    if (!(appletGetAppletType() == AppletType_Application)) {
        printf(CONSOLE_RED "Switchbru Launcher is not running in application mode.\n");
        printf("Launch the launcher from application mode instead.\n");
        printf(CONSOLE_YELLOW "If you are using Atmosphere,\nlaunch via holding R while opening an application\n\n");
        isApp = false;
    } else {
        printf(CONSOLE_GREEN "Press A to relaunch Switchbru Launcher.\n\n");
    }
    printf(CONSOLE_RESET "Copyright (c) Syntax64 \"Syntax 64\" V., 2021\n\nReleased under the AGPLv3.\nNot licensable under later versions and clause 7b and 7c are in effect.");
